...
- Разработка [тест-кейсов](https://miro.com/app/board/uXjVLMrcM5w=/?share_link_id=317306456649 ) в TMS системе Test IT для проверки авторизации на сайте-визитке. 
...
5. Перейдите в терминале в папку с репозиторием, используя команду:

```html
    cd путь
```

Все новые тесты добавляются в папку:
...
6. В терминале в папке с проектом для открытия Cypress выполните:
...
7. В разделе `Specs` выбери `Authorization.cy.js` и запустите автотесты кнопкой `Run All Tests`, либо на клавиатуре нажмите `R`
